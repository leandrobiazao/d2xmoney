            ...
            if subtype_allocations.exists():
                ...
                for subtype_alloc in subtype_allocations:
                    ...
                    if is_crypto_subtype and type_alloc.investment_type.code == 'RENDA_VARIAVEL_DOLARES':
                        ...
                        # Check for ETF Renda Fixa stock allocations
                        if subtype and subtype.code == 'ETF_RENDA_FIXA':
                            stock_allocations = subtype_alloc.stock_allocations.all()
                            for stock_alloc in stock_allocations:
                                etf_stock = stock_alloc.stock
                                if not etf_stock or not etf_stock.is_active:
                                    continue
                                
                                # Calculate target value for the ETF stock (use subtype percentage)
                                etf_target_value = subtype_target_value
                                
                                # Get current position for this ETF stock
                                ...
                                # Create action for ETF Renda Fixa stock
